...
const CommandEntry commands[7] = {{"engine", _get_set_engine_entry_func}, {"exit", _exit_daemon_entry_func}, {"list-engine", _list_engine_entry_func}, {"watch", _message_watch_entry_func}, {"restart", _restart_daemon_entry_func}, {"version", _print_version_entry_func}, {"help", _print_help_entry_func}};
...
gint list_engine (gchar** argv, int argv_length1) {
	...
	_tmp0_ = _ ("command [OPTIONS]");
	_tmp1_ = g_option_context_new (_tmp0_);
	option = _tmp1_;
	_tmp2_ = option;
	g_option_context_add_main_entries (_tmp2_, options, "ibus");
	{
		GOptionContext* _tmp3_;
		_tmp3_ = option;
		g_option_context_parse (_tmp3_, &argv_length1, &argv, &_inner_error_);
	...
	if (_tmp8_ == NULL) {
		...
		_tmp9_ = stderr;
		_tmp10_ = _ ("Can't connect to IBus.\n");
	...
	_tmp40_ = g_hash_table_get_keys (_tmp39_);
	{
		...
		for (language_it = language_collection; language_it != NULL; language_it = language_it->next) {
			...
			language = (const gchar*) language_it->data;
			{
				...
				list = _tmp44_;
				_tmp45_ = _ ("language: %s\n");
				_tmp46_ = language;
				_tmp47_ = ibus_get_language_name (_tmp46_);
				...
				_tmp49__length1 = _tmp48_->data_length1;
				{
					...
					for (engine_it = 0; engine_it < _tmp49__length1; engine_it = engine_it + 1) {
...
gint exec_setxkbmap (IBusEngineDesc* engine) {
	...
	}
	{
		...
		if (!_tmp54_) {
			...
			g_warning ("main.vala:124: Switch xkb layout to %s failed.", _tmp56_);
			result = EXIT_FAILURE;
			args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
			_g_free0 (standard_error);
			_g_free0 (option);
	...
	__catch1_g_spawn_error:
	{
		...
		g_warning ("main.vala:129: Execute setxkbmap failed: %s", _tmp58_);
		result = EXIT_FAILURE;
		_g_error_free0 (e);
		args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
		_g_free0 (standard_error);
	...
	if (_tmp59_ != 0) {
		...
		_tmp65_ = _tmp62_;
		g_warning ("main.vala:134: Execute setxkbmap failed: %s", _tmp65_);
		result = EXIT_FAILURE;
		_g_free0 (_tmp62_);
		args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
		_g_free0 (standard_error);
...
gint _vala_main (gchar** argv, int argv_length1) {
	...
	_new_argv_size_ = new_argv_length1;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp8_ = FALSE;
				_tmp11_ = i;
				if (!(_tmp11_ < G_N_ELEMENTS (commands))) {
					break;
				}
				_tmp12_ = i;
				...
				if (g_strcmp0 (_tmp14_, _tmp16_) == 0) {
					...
					_tmp21_ = _tmp19_ (_tmp20_, _tmp20__length1, _tmp19__target);
					result = _tmp21_;
	...
	_tmp22_ = stderr;
	_tmp23_ = _ ("%s is unknown command!\n");
	_tmp24_ = argv;
