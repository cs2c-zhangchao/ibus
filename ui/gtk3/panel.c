...
static void panel_exec_setxkbmap (Panel* self, IBusEngineDesc* engine) {
	...
	}
	{
		...
		if (!_tmp54_) {
			...
			g_warning ("panel.vala:350: Switch xkb layout to %s failed.", _tmp56_);
		}
	}
	goto __finally3;
	__catch3_g_spawn_error:
	{
		...
		g_warning ("panel.vala:354: Execute setxkbmap failed: %s", _tmp58_);
		_g_error_free0 (e);
	}
	__finally3:
	if (_inner_error_ != NULL) {
	...
	if (_tmp59_ != 0) {
		...
		_tmp65_ = _tmp62_;
		g_warning ("panel.vala:358: Execute setxkbmap failed: %s", _tmp65_);
		_g_free0 (_tmp62_);
	}
	args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
	_g_free0 (standard_error);
...
static void panel_switch_engine (Panel* self, gint i, gboolean force) {
	...
	if (!_tmp16_) {
		...
		g_warning ("panel.vala:372: Switch engine to %s failed.", _tmp18_);
...
static void panel_handle_engine_switch (Panel* self, GdkEvent* event, gboolean revert) {
	...
	if (_tmp19_) {
		...
		_tmp31_ = i;
		if (_tmp31_ < 0) {
			g_debug ("panel.vala:435: switch cancelled");
		} else {
			gint _tmp32_;
...
static void panel_show_setup_dialog (Panel* self) {
	...
	__catch4_g_spawn_error:
	{
		...
		g_warning ("panel.vala:525: Execute %s failed! %s", _tmp10_, _tmp12_);
		self->priv->m_setup_pid = (GPid) 0;
		_g_error_free0 (e);
	}
	__finally4:
